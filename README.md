...
- **Move face — `M`.** Click a flat surface to select its coplanar face; the camera **rotates to a head-on (planar) view** and a 3-axis gizmo aligned to the surface appears. Drag an axis to **push/pull** — the vertices on that face (and every triangle welded to them) move, so the solid restretches to the new face position. Bounding-box dimensions update live. **Refine the selection** head-on with **Ctrl+drag** (add a rectangle) / **Shift+drag** (remove a rectangle); rectangle corners **snap to strong edges** (sharp/boundary), and if you keep moving on and off a snap it disengages for that drag.
